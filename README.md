...
| vos.Service.FoodBalance | Production | Reads the productive footprint and the yield the shared study archetype declares to work out how many residents that land feeds and what share of the population that is. It asserts nothing: both figures are formulas the study declares |
| vos.Service.RainwaterHarvest | Production | Reads the built footprint, the site's rainfall and the runoff the shared study archetype declares, and serves each demand the model declares in the order it declares, each demand's size being a figure the study works out — writing the total asked for, the share of each covered and the volume still short |
