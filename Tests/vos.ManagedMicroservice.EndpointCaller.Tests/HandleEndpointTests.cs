...
// Tests for the /handle endpoint in vos.ManagedMicroservice.EndpointCaller/Program.cs.
...
public class HandleEndpointTests
{
    ...
    public async Task Health_ReturnsHealthy()
    {
        ...
        body.Should().Contain("\"service\":\"EndpointCaller\"");
    }
    ...
    public async Task Shutdown_Returns200WithMessage()
    {
        ...
        response.StatusCode.Should().Be(HttpStatusCode.OK);
        var body = await response.Content.ReadAsStringAsync();
        body.Should().Contain("Shutting down EndpointCaller");
    }
