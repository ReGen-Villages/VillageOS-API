// Targeted tests covering specific branches in vos.ManagedMicroservice.Delta
...
public class CoverageGapTests
{
    ...
    {
        var factory = new DeltaWebApplicationFactory();
        await factory.InitializeAsync();
        var isId = Guid.NewGuid();
        var defaultId = Guid.NewGuid();
        factory.HandlerCallback = req =>
    ...
    private sealed class MalformedSeedFactory : DeltaWebApplicationFactory
    {
    ...
    private sealed class AuthEnabledFactory : DeltaWebApplicationFactory
    {
        public new Task InitializeAsync()
        {
            base.InitializeAsync().GetAwaiter().GetResult();
            Environment.SetEnvironmentVariable("DELTA_SIGNING_KEY",
                Convert.ToBase64String(new byte[32]));
            Environment.SetEnvironmentVariable("DELTA_ISSUER", "VillageOS");
            Environment.SetEnvironmentVariable("DELTA_AUDIENCE", "VosClients");
            return Task.CompletedTask;
        }

        public new Task DisposeAsync()
        {
            Environment.SetEnvironmentVariable("DELTA_SIGNING_KEY", null);
            Environment.SetEnvironmentVariable("DELTA_ISSUER", null);
            Environment.SetEnvironmentVariable("DELTA_AUDIENCE", null);
