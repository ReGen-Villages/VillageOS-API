// Targeted tests covering specific branches in vos.ManagedMicroservice.Tributary that
...
public class CoverageGapTests
{
    ...
    private sealed class AuthEnabledFactory : TributaryWebApplicationFactory
    {
        public new Task InitializeAsync()
        {
            base.InitializeAsync().GetAwaiter().GetResult();
            Environment.SetEnvironmentVariable("TRIBUTARY_SIGNING_KEY",
                Convert.ToBase64String(new byte[32]));
            Environment.SetEnvironmentVariable("TRIBUTARY_ISSUER", "VillageOS");
            Environment.SetEnvironmentVariable("TRIBUTARY_AUDIENCE", "VosClients");
            return Task.CompletedTask;
        }

        public new Task DisposeAsync()
        {
            Environment.SetEnvironmentVariable("TRIBUTARY_SIGNING_KEY", null);
            Environment.SetEnvironmentVariable("TRIBUTARY_ISSUER", null);
            Environment.SetEnvironmentVariable("TRIBUTARY_AUDIENCE", null);
