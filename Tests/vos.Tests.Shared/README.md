...
- `Settle.UntilAsync(condition, expectation)` — waits for something a test cannot await: a background loop's effect, a fire-and-forget write. Use it wherever the alternative is `await Task.Delay(someNumber)`. A fixed wait is an assertion about how busy the build agent is, so it passes on an idle machine and reddens an unrelated pull request on a loaded one; this waits as long as the work actually takes and gives up only after a ceiling long enough that reaching it means a genuine hang.
