...
// Stands in for Mycelium in a handler's tests: holds a key pair on the P-256 curve, hands
// out the public half exactly as a daemon receives it, and signs the tokens a handler is meant to
// accept — and the ones it must refuse.
public sealed class MyceliumSigner
{
    private readonly ECDsa _pair = ECDsa.Create(ECCurve.NamedCurves.nistP256);

    // The public half, encoded as the VerificationKey setting a daemon is launched with.
    ...
    // What someone who reads the verification key off a daemon can produce: a token signed
    // with that key used as a plain shared secret. A handler that accepted it would have handed an
    // attacker a signing key.
