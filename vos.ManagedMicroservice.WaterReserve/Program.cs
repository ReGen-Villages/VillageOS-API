...
using vos.ManagedMicroservice.WaterReserve.Configuration;
...
try
{
    ...
    var handle = app.MapPost("/handle", async (HttpContext ctx, WaterReserveNode node) =>
    {
        ...
        return Results.BadRequest(new { error = "WaterReserve is a DAG node; expected a node envelope (runId, nodeId)." });
    });
