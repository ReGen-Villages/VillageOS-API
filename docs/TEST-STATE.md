...
- Same `partial class Program {}` + Testing-env Serilog guard + `IConfiguration`-based CliArgs fallback as the other two microservices, including the `PerCallHttpClientFactory` from Phase 2C. Tests that construct a `DeltaWebApplicationFactory` opt into `[Collection(nameof(DeltaFactoryCollection))]` so the synthetic `seed.json` written into `AppContext.BaseDirectory` doesn't get clobbered by parallel factories; a per-test seed path would lift the constraint.
