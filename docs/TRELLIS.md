...
the answers now live in the model rather than in the browser.
...
- **The parcel boundary is drawn on the map, and its area is measured on the sphere.** The planner
  places a draft square of the stated area centred on the site, or clicks corners onto the map, and
  drags any corner; the drawn area is computed on the sphere (`src/utils/parcelGeometry.ts` — flat
  maths reads a northern-Europe parcel at nearly twice its size) and compared with the stated area.
  Within the named tolerance (`AREA_MATCH_TOLERANCE`) the two read as a match; outside it the page says
  how far apart they are. The boundary is posted with how it was obtained — a generated square must not
  read as a surveyed one — as a term the model declares (`drawn-by-hand`,
  `generated-from-stated-area`), and any corner the planner places or moves makes the whole boundary
  drawn by hand.
...
the browser, so the page says to open it and copy the numbers rather than failing silently. Once both
coordinates are given, the shared map module ([§22](#22-the-map-and-its-basemap-sources)) shows the
site with a marker, and the parcel step draws on that same map.
...
└── src/
    ...
    │   ├── SurveysStep.tsx      # The surveys asked for after the report, sent one by one under the ticket
    │   ├── sharedSurveys.ts     # The chosen files, how far each has got, and what the model lists
    │   ├── OverviewView.tsx     # The closing view: the parcel on the imagery, the tab sections in a sheet
    │   └── overviewState.ts     # Which sections are tabs, and which basemap the view opens on
    │
    └── components/
        ├── layout/
...
**Every route above renders only after somebody has signed in.** `App.tsx` answers an unauthenticated
...
- **A row arrives with the columns its table draws.** `stateList` names them in
  ...
  table costs the rows it shows rather than a read of every Thing in the model.

  The page still asks its subscription for that type, because two things read the
  row's own Thing out of what the page holds: a `computed` column resolves with
  that Thing as its scope and walks its edges, and a detail card opened on the row
  reads its properties and relations. Both would have to ask for what the page
  does not hold before the type could be dropped.

  Three things the platform decides, not the client:
  - **An inherited value reaches the row like an owned one.** The endpoint reads
    each name own-first and then up the `is` chain.
  - **A name the Thing does not hold is absent from the row**, rather than
    present and empty — so "no value" reads differently from "the value is
    nothing", and a column bound to it renders as absent.
  - **A name the Thing holds by two inheritance paths is refused**, not guessed
    at: the whole read fails, saying which Thing and which name.

  A binding that names nothing asks for nothing more than the id and the name, so
  a table drawing a column its binding does not name shows that column empty. A
  `computed` column is the exception and is still derived here — it walks edges,
  which is not a question this endpoint answers.
...
`lineSeries` draws several series over one calendar axis — one line with a point per group each,
on one scale — the monthly means of the daily high, mean and low across the window.

```json
{ "type": "lineSeries", "title": "Average monthly temperature", "unit": "°C", "format": "decimal1",
  "series": [
    { "label": "Daily high", "value": { "kind": "history", "property": "temperature", "windowSeconds": 347155200,
                                        "steps": [ { "fold": "day", "function": "Max" }, { "fold": "month", "function": "Average" } ] } },
    { "label": "Daily mean", "value": { "kind": "history", "property": "temperature", "windowSeconds": 347155200,
                                        "steps": [ { "fold": "month", "function": "Average" } ] } } ] }
```

Each series is a `history` binding folded by a calendar period, so its keys order along the axis;
the axis is labelled once per year, at the first group of each. Series take the categorical palette
slots in the order listed — eight validated slots, and a ninth series is not drawn — and the legend
names them; a series the platform answered nothing for is left out of the chart and the legend.
Every series shares one scale: two measures that need two scales are two charts.
...
A row is `{ id, name, ... }` and what follows is what the row's own Thing
stores — its effective properties for `thingList` and `compareEntities`, read
from the model index, and for `stateList` the columns it named, sent by the
platform (see [Narrowing a state answer where it is answered](#narrowing-a-state-answer-where-it-is-answered)).
Either way a column can only show what that one Thing holds. Three parts of the
contract lift that limit.
...
A source that declares the last four draws in three dimensions (#6912, with platform Feature #6911 declaring them): the ground is raised from the elevation tiles, the buildings the basemap already carries are raised out of it, and a control tilts the camera. Every map, whatever its source declares, is drawn on a globe under a sky and shows where north lies. A source declaring none of the four draws flat, exactly as every source did before, and no tilt is offered for a view the model cannot draw.

- **The globe and the sky need no declaration.** Neither names a provider, so both are the library's own wherever it can draw them, which is every map this client builds. The globe is the library's blend from a globe far out to the flat plane close in, so a parcel is looked at exactly as before and a page opening on the world opens on the Earth. The sky is what stands above the horizon when the camera leans far enough to see one, and the atmosphere round the globe from far out. Both are style properties, wiped by a style swap as the ground is, and each is put back once per style: each setter repaints the map, which settles into the idle they are put back on, and a map putting them back on every one of those would never stop drawing.
- **The camera leans to the horizon only where there is land to look across.** The library's own ceiling never brings the horizon into view, so a sky over the land would go unseen; where the model declares something to raise, the ceiling is lifted far enough to see it. A flat map keeps the library's ceiling, and switching to one brings the camera back under it.
- **Turning the map is the library's own gesture** — a drag with the second mouse button, or two fingers on a touch screen. The page adds a control that shows where north lies as the map turns, and turns it back.
