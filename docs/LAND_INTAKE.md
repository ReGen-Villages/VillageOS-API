...
| 4 | ~~**Retention** for submissions that are never promoted.~~ **Settled: every submission is retained.** A rejected one moves to cold storage 30 days after it was rejected; one nobody has dealt with is kept indefinitely. The period lives on the disposition Thing (`daysBeforeColdStorage` on `rejected`), so changing it is a model edit, and a disposition naming no period is kept. | What remains is the mechanism, not the rule. Cold storage in this platform is Heartwood, which holds history — observations and property Facts — not Things, and nothing today moves a Thing out of a live model into it. That belongs with the intake model itself (VillageOS #6044). |
