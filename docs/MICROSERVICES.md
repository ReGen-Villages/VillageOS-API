...
both also run **reactively** (#5839) — a graph `/handle` whose subject is the site anchor makes the service
read its inputs off the anchor's effective properties, compute, and write its outputs back as Facts, so the
anchor's judge ranges re-evaluate (no pipeline). `ModelBridge` (#5866) is a generic
