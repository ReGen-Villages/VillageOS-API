...
It's the Node analogue of the canonical C# [`vos.ManagedMicroservice.CSharp.Echo`](../vos.ManagedMicroservice.CSharp.Echo) — an **echo handler**: `/handle` acknowledges the relationship and reflects the payload back. Replace the `/handle` block in `src/index.ts` with your own logic.
