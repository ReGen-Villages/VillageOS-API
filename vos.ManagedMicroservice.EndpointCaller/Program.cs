...
using vos.ManagedMicroservice.EndpointCaller.Models;
...
var logPath = Path.Combine(AppContext.BaseDirectory, "..", "..", "..", "..", "logs", "endpointcaller-.log");
Log.Logger = new LoggerConfiguration()
    .MinimumLevel.Information()
    .MinimumLevel.Override("Microsoft.AspNetCore", Serilog.Events.LogEventLevel.Warning)
    .Enrich.FromLogContext()
    .Enrich.WithProperty("Service", "EndpointCaller")
    .WriteTo.File(
        path: logPath,
...
try
{
    Log.Information("VillageOS EndpointCaller Service - Port: {Port}, Broker: {BrokerUrl}", servicePort, brokerUrl);
    ...
    app.MapGet("/health", () => Results.Ok(new { status = "Healthy", service = "EndpointCaller" }));
    ...
    var shutdownEndpoint = app.MapPost("/shutdown", (IHostApplicationLifetime lifetime) =>
    {
        ...
        return Results.Ok(new { message = "Shutting down EndpointCaller" });
    });
...
{
    Log.Fatal(ex, "EndpointCaller terminated unexpectedly");
}
finally
{
    Log.CloseAndFlush();
