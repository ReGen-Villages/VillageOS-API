...
// marks an input the node consumes as a list — the node fans out over it.
...
public sealed class DagNode
{
    ...
    // Input-port → run-param-key bindings: at dispatch each bound input is filled from the
...
// and place it at a field of the downstream input, so several wires can compose one input. An optional
