...
describe('i18n instance', () => {
  ...
  it('resolves a translated key in the active language', async () => {
    ...
    expect(i18n.t('navigation.logs')).toBe('Registros');
  });
  ...
  it('falls back to the base locale for a key missing from the target locale', () => {
    ...
    i18n.addResourceBundle('zz', 'translation', { navigation: { logs: 'ZZ logs' } });
    expect(i18n.t('navigation.logs', { lng: 'zz' })).toBe('ZZ logs');
    expect(i18n.t('navigation.things', { lng: 'zz' })).toBe(i18n.t('navigation.things', { lng: 'en' }));
    i18n.removeResourceBundle('zz', 'translation');
  });

  it('resolves both Arabic regions through the shared ar translation', () => {
    expect(i18n.t('navigation.logs', { lng: 'ar-SA' })).toBe('السجلات');
    expect(i18n.t('navigation.logs', { lng: 'ar-AE' })).toBe('السجلات');
  });
