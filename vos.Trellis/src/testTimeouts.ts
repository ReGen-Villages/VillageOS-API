/** Vitest's default of five seconds is a bound on how busy the build agent is rather than on anything a
 *  test body does. The agent is shared, and under contention it has been measured creating each test
 *  file's environment in three to four and a half seconds against about a third of a second on a developer
 *  machine — ten to fifteen times slower. A body that renders the whole submission form and then drives it
