...
export function AddRelationshipRow({ direction, fixedThingId, things, relationships, onCreated }: Props) {
  ...
  }, [canSubmit, direction, fixedThingId, otherId, predicateId, onCreated]);

  return (
    <div className="border-t border-zinc-700/50 mt-2 pt-2 space-y-1.5">
      <ThingPicker things={predicateSorted} value={predicateId} onChange={setPredicateId} placeholder="Predicate..." />
      <ThingPicker things={nonPredicateThings} value={otherId} onChange={setOtherId} placeholder={direction === 'outgoing' ? 'Target...' : 'Subject...'} />
