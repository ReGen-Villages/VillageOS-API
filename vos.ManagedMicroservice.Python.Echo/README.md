...
A complete, runnable VillageOS handler written in **Python with FastAPI**. It implements the full managed-microservice contract documented in [docs/MICROSERVICE_AUTHORING.md](../docs/MICROSERVICE_AUTHORING.md): startup registration, the four required endpoints, inbound JWT validation (PyJWT), and graceful deregistration.

It's the Python analogue of the canonical C# [`vos.ManagedMicroservice.CSharp.Echo`](../vos.ManagedMicroservice.CSharp.Echo) — an **echo handler**: `/handle` acknowledges the relationship and reflects the payload back. Replace `handle_relationship()` in `app.py` with your own logic.
