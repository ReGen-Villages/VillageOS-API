...
// What a recompute answers: the figures it worked out with nothing outstanding, or no figures and
// the inputs the study holds no number under yet.
//
// A dispatch that finds an input missing has not failed. A study a submission built carries land, a
// boundary and programme shares, so a reservoir capacity or a panel area is absent until a building model
// exists — and a handler that threw on one had the broker record __DispatchState=Failed and re-drive
...
// input lands and the watch on the study wakes it.
