...
{
    // Null when the token cannot be read, names no model, or states no expiry. A token with no
    // stated end would otherwise be held forever and fail only once something depended on it.
